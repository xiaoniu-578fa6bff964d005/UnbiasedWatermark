...
for i in range(num_batches):
    ...
    batch_translation = model.generate(torch.Tensor(batch['inputs']).to(device=model.device).long(),
                             max_length=512)
...
bleu_scores = metric.compute(predictions=generated_translations, references=tokenized_wmt["de"])
