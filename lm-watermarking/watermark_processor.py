...
class WatermarkBase:
    ...
    def __repr__(self):
        return f"WatermarkLogitsProcessor_John({repr(self.vocab_size)}, {repr(self.gamma)}, {repr(self.delta)},{repr(self.seeding_scheme)},{repr(self.hash_key)},{repr(self.select_green_tokens)})"
    ...
    def _get_greenlist_ids(self, input_ids: torch.LongTensor) -> list[int]:
        ...
        #Changed to self.vocab_size-1 for index out of range error
#         vocab_permutation = torch.randperm(self.vocab_size, device=input_ids.device, generator=self.rng)
        vocab_permutation = torch.randperm(self.vocab_size-1, device=input_ids.device, generator=self.rng)
...
#         print("XDXDXDXDDDDDDDDDDXDXDXD")
#         print(scores.shape,max(greenlist_token_ids[0]))
#         print(greenlist_token_ids)
        green_tokens_mask = torch.zeros_like(scores)
        for b_idx in range(len(greenlist_token_ids)):
#             print(scores.shape,max(greenlist_token_ids[b_idx]))
